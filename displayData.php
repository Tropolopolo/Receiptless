<?php
  ...
  else
  {
    ...
    if($email)
    {
        $select = "select PhoneNumber from account where Email='".$email."'";
        $rs = $conn->prepare($select);
        $rs->execute();
        $phoneNum = $rs->fetchALL(PDO::FETCH_ASSOC);
        foreach($phoneNum as $num)
        {
			$userNum = $num['PhoneNumber'];
        }
          
        $query = "select * from receiptdata Where PhoneNumber='".$userNum."'";
        $rs = $conn->prepare($query);
        $rs->execute();
        $entry = $rs->fetchALL(PDO::FETCH_ASSOC);
        if($entry)
        {
			$str.= "<div class='grid-item' style='font-weight: bold'>Store Name<br>
					Phone Number<br>
					Date<br>
					Time<br>
					Total<br>
			...
			{
				$str.= "<div class='grid-item'>".$row['StoreName']."<br>";
				$str.= ''.$row['PhoneNum'].'<br>';
				$str.= ''.$row['RecDate'].'<br>';
				$str.= ''.$row['RecTime'].'<br>';
				$str.= ''.$row['Total'].'<br>';
				$str.= ''.$row['ReceiptID'].'</div>';
			}
			$str.= '</div>';
        }
        else {
