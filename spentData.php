<?php
  ...
  else
  {
    ...
    if($sort&&$dollar)
    {
      ...
      $str.="<div class='grid-item' style='font-weight: bold'>
            User Email<br>
            Amount Spent<br>
      ...
      {
        $select2 = "select Email from account where PhoneNumber ='".$user['phoneNum']."'";
        $rs = $conn->prepare($select2);
        $rs->execute();
        $users2 = $rs->fetchALL(PDO::FETCH_ASSOC);
        foreach($users2 as $user2)
        {
          $str.="<div class='grid-item'>".$user2['Email'].'<br>';
        }
        $str.= '$'.number_format($user['total'],2).'<br></div>';
      }
      $str.= '</div>';

      }
      else{
        $str.= "<p>One or more parameters is/are missing</p>";
      }
	fwrite($myfile, $str);
	fclose($myfile);
  }
